...
class Produk:
    ...
    def get_user_pp_link(user_id):
        ...
        try:
            ...
            if user_record:
                return user_record['pp_link_img']
            else:
                return None
    ...
    @staticmethod
    def update_user_pp_link(user_id, new_pp_link):
        conn = get_db()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        ...
        try:
            query = """
                ...
                RETURNING *; 
            """
            cursor.execute(query, (new_pp_link, user_id))
            
            updated_user = cursor.fetchone()

            if updated_user:
                conn.commit()
                return dict(updated_user) # Kembalikan data yang sudah diupdate
            else:
                return None

        except Exception as e:
            conn.rollback()
            print(f"Terjadi error, transaksi dibatalkan: {e}")
            raise e
        finally:
            cursor.close()
